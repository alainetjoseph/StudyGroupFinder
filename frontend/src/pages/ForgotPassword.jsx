...
export default function ForgotPassword() {
  ...
  async function handleSubmit(e) {
    ...
    try {
      ...
      // Optionally redirect to login or show instruction
      // navigate("/login");
    } catch (err) {
      console.log(err);
      enqueueSnackbar(
  ...
  return (
    <div className="min-h-screen w-full flex flex-col lg:flex-row bg-background">
      {/* LEFT PANEL - Reusing Login style */}
      <div className="hidden lg:flex w-1/2 relative flex-col justify-between p-16 overflow-hidden">
        ...
        <div className="relative z-10 flex items-center gap-2">
          ...
          <span className="text-2xl font-bold tracking-tight text-foreground" onClick={() => navigate("/")} style={{ cursor: 'pointer' }}>
            StudyGroupFinder
          </span>
        </div>
        ...
        <div className="w-full max-w-md">
          ...
          <div className="mb-8">
            ...
            <p className="mt-2 text-sm text-muted">
              No problem! Just enter your email below.
            </p>
          </div>
          ...
          <p className="mt-10 text-center text-sm text-muted">
            ...
            <a href="/login" className="ml-1 text-primary hover:text-foreground">
              Back to Login
            </a>
          </p>
        </div>
